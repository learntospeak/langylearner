// lesson-shim.js ΓÇö with Pronounce + Romaji toggle
window.LessonShim = (() => {
  // ---------- utils ----------
  const norm = (s) => (s || "").replace(/[πÇé∩╝ÄπÇü∩╝î∩╝ü∩╝ƒ!\?()\[\]{}'"πÇîπÇìπÇÄπÇÅπâ╗ΓÇª∩╝Ü:∩╝¢;\-∩╝┐_πÇ£~πâ╝\s]/g, "").toLowerCase();
  ...
  const H = {
    ...
    makeHint(expectedJP, gotRaw, sentence) {
      ...
      if (!gotRaw || !gotRaw.trim())
        return `Type your answer. Hint: starts with πÇî${exp.slice(0, 2)}πÇì`;

      // Common pitfalls
      if (expectedJP.includes("πüôπéôπü½πüíπü»") && got.includes("πüôπéôπü½πüíπéÅ"))
        return "Use πÇîπü»πÇì (ha) not πÇîπéÅπÇì in πüôπéôπü½πüíπü».";

      if (/Θíÿ/.test(expectedJP) && got === "πüèπü¡πüîπüäπüùπü╛πüÖ")
        return ""; // accept kana reading for Θíÿ

      if (exp.includes("πüú") && !got.includes("πüú")) {
        const idx = exp.indexOf("πüú"), next = exp[idx + 1] || "";
        return `Add a small πÇîπüúπÇì before πÇî${next}πÇì.`;
      }
      ...
      if (i >= 0) return `Check character ${i + 1}: should be πÇî${exp[i]}πÇì.`;

      return "Check particles/spelling.";
    },
  };
  // --- MORA HELPERS ---
  const SMALL_KANA = "πéâπéàπéçπâúπâÑπâºπüüπüâπüàπüçπüëπéíπéúπéÑπéºπé⌐πéÄπâ«";
  function splitMora(str = "") {
    const a = [...str]; const out = [];
    for (let i = 0; i < a.length; i++) {
      const c = a[i], n = a[i + 1];
      if (c === "πüú" || c === "πââ" || c === "πâ╝") { out.push(c); continue; }
  ...
  function attachKanaGuide(inp, targetJP, map, readingOverride = null) {
    ...
    const kanaOnly = s => /^[\p{sc=Hiragana}\p{sc=Katakana}πâ╝]+$/u.test(s || "");
    ...
    const draw = (p) => {
      ...
      foot.textContent = next ? `Next: ${next} (${r})` : 'Γ£ô Complete';
    };
  ...
  // === Canonical readings + helpers (ONE copy only) ===
  const READINGS = {
    "πüèΘíÿπüäπüùπü╛πüÖ": "πüèπü¡πüîπüäπüùπü╛πüÖ",
    "πüèτû▓πéîµºÿπüºπüÖ": "πüèπüñπüïπéîπüòπü╛πüºπüÖ",
    "πüèτû▓πéîµºÿ": "πüèπüñπüïπéîπüòπü╛",
    "τû▓πéîµºÿ": "πüñπüïπéîπüòπü╛"
    // add more as neededΓÇª
  };
  ...
  function normalizeKanaToExpected(typedRaw, expectedRaw) {
    ...
    const clean = s => toHira((s || "").replace(/[^\p{sc=Hiragana}\p{sc=Katakana}πâ╝]+/gu, ""));
    ...
    const bigOf = { "πéâ": "πéä", "πéà": "πéå", "πéç": "πéê" };
    ...
    while (i < e.length && j < t.length) {
      ...
      // Particles: πü»/πü╕/πéÆ typed as πéÅ/πüê/πüè
      if ((em === "πü»" && tm === "πéÅ") || (em === "πü╕" && tm === "πüê") || (em === "πéÆ" && tm === "πüè")) {
        out.push(em); i++; j++; continue;
      }

      // Small ya/yu/yo: πüì + πéä ΓåÆ πüìπéâ (when expected has small)
      const m = em.match(/^([πüìπüÄπüùπüÿπüíπüóπü½πü▓πü│πü┤πü┐πéè])([πéâπéàπéç])$/);
      if (m && tm === m[1] && t[j + 1] === bigOf[m[2]]) {
        out.push(em); i++; j += 2; continue;
      }

      // N-ambiguity: πéô + πüä ΓåÆ πü½ ; πéô + (πéä/πéå/πéê) ΓåÆ πü½πéâ/πü½πéà/πü½πéç
      if (em === "πü½" && tm === "πéô" && t[j + 1] === "πüä") {
        out.push("πü½"); i++; j += 2; continue;
      }
      if (["πü½πéâ", "πü½πéà", "πü½πéç"].includes(em) && tm === "πéô" && ["πéä", "πéå", "πéê"].includes(t[j + 1])) {
        const want = { "πü½πéâ": "πéä", "πü½πéà": "πéå", "πü½πéç": "πéê" }[em];
        if (t[j + 1] === want) { out.push(em); i++; j += 2; continue; }
      }

      // Small πüú missing: if expected has πüú and next typed mora matches next expected
      if (em === "πüú" && t[j] === e[i + 1]) {
        out.push("πüú"); i++; continue;
      }

      // Long vowel normalization: πüèπüè vs πüèπüå (follow expected)
      if (em === "πüå" && out[out.length - 1] === "πüè" && tm === "πüè") {
        out.push("πüå"); i++; j++; continue;
      }
  ...
  // Convert romaji -> πü▓πéëπüîπü¬ and fix greeting edge cases on each keystroke
  // Convert romaji -> πü▓πéëπüîπü¬ and fix greeting edge cases per keystroke
  ...
  function attachGreetingNormalizer(inp, expectedJP = "") {
    inp.addEventListener("input", () => {
      ...
      // safe global fixes
      after = after.replace(/πüôπéôπüäπüíπéÅ/g, "πüôπéôπü½πüíπü»");
      after = after.replace(/πüôπéôπü░πéôπéÅ/g, "πüôπéôπü░πéôπü»");

      // enforce πü» only when that phrase is expected
      if (expectedJP.includes("πüôπéôπü½πüíπü»")) after = after.replace(/πüôπéôπü½πüíπéÅ/g, "πüôπéôπü½πüíπü»");
      if (expectedJP.includes("πüôπéôπü░πéôπü»")) after = after.replace(/πüôπéôπü░πéôπéÅ/g, "πüôπéôπü░πéôπü»");
  ...
  function bindNDisambiguator(inp, expectedJP) {
    ...
    const active = /πüôπéôπü½πüíπü»|πüôπéôπü░πéôπü»/.test(expectedJP || "");
    if (!active) return;
  ...
  function normalizeToExpected(el, expectedJP) {
    ...
    let v = el.value || "";
    if (expectedJP.includes("πüôπéôπü½πüíπü»")) v = v.replace(/πüôπéôπüäπüíπéÅ|πüôπéôπü½πüíπéÅ/g, "πüôπéôπü½πüíπü»");
    if (expectedJP.includes("πüôπéôπü░πéôπü»")) v = v.replace(/πüôπéôπü░πéôπéÅ/g, "πüôπéôπü░πéôπü»");
    el.value = v;
  }
  ...
  function renderReadListen(lesson, step, map) {
    ...
    (step.item_refs || []).forEach((sid) => {
      ...
      row.innerHTML = `
        <div class="flex items-start gap-2">
          <button type="button" class="${map?.classes?.speakBtn || "speak-btn"}" data-jp="${s.jp}">≡ƒöè</button>
          <div>
    ...
    setStatus(map, "Read, listen, and repeat. Use ≡ƒöè or Speak.");
    feedback(map, "", true);
  }
  ...
  size="${Math.max(2, Math.min(12, (b || "").length))}" />`;
    ...
    `;
      ...
      blanks.forEach(inp => {
        ...
        const reading = kanjiToReading(exp);            // "πüèΘíÿπüäπüùπü╛πüÖ" -> "πüèπü¡πüîπüäπüùπü╛πüÖ"
        inp.dataset.expectedKana = reading;

        // 3) show kana chips + next-hint using the kana reading
        attachKanaGuide(inp, exp, map, reading);

        // 3) gentle greeting fixes (πüôπéôπü½πüíπü» / πüôπéôπü░πéôπü»), if you added it
    ...
    (step.item_refs || [])
      ...
      .forEach(s => {
        ...
        <input class="${map?.classes?.input || "jp-input"}"
               data-expected="${s.jp || ""}"
               placeholder="Type in JapaneseΓÇª" />
        ${map?.flags?.showRomaji ? `<div class="${map?.classes?.romaji || "romaji"}">${s.romaji_full || ""}</div>` : ""}
        <div class="${map?.classes?.hint || "hint"} text-sm text-amber-700 mt-1"></div>
      `;
  ...
  function renderTranslateSyllables(lesson, step, map) {
    ...
    (step.item_refs || [])
      ...
      .filter(Boolean)
      .forEach(s => {
        // Use the sentence reading so kanji like Θíÿ become πüèπü¡πüîπüäΓÇª
        const reading = sentenceReadingHira(s);
        ...
        inputs.forEach((inp, idx) => {
          ...
          inp.addEventListener("input", () => {
            ...
            if (kana === exp) {
              ...
              hintEl.textContent = `This syllable is πÇî${exp}πÇì (${romaji}).`;
            }
          });

          // Backspace on empty ΓåÆ reopen previous box
  ...
  function renderRoleplay(lesson, step, map) {
    ...
    const turns = (step.item_refs || [])
      ...
      .map(s => ({
        promptEN: s.en || '',
        expectJP: s.jp || '',
        reading: sentenceReadingHira(s) // e.g., πüèπü¡πüîπüäπüùπü╛πüÖ
      }));
    ...
    <div class="text-sm text-gray-600 mb-2">Say it in Japanese. Click ≡ƒÄñ and speak.</div>
    <div class="flex items-center gap-2 mb-2">
      <button class="btn btn-primary" data-act="speak">≡ƒöè Play Prompt</button>
      <button class="btn btn-amber"   data-act="rec">≡ƒÄñ Start</button>
      <button class="btn btn-ghost"   data-act="skip">ΓÅ¡ Skip</button>
    </div>
    ...
    async function startRec() {
      if (!hasRecognition() || busy) {
        elHint.textContent = hasRecognition()
          ? 'Recording in progressΓÇª'
          : 'Speech recognition is not supported in this browser.';
        return;
      }
      busy = true;
      elHint.textContent = 'ListeningΓÇª';
      elHeard.textContent = '';
      ...
      rec.onresult = (evt) => {
        ...
        feedback(map, ok ? 'Great pronunciation!' : 'CloseΓÇötry again.', ok);
      ...
      rec.onerror = () => { elHint.textContent = 'DidnΓÇÖt catch that. Try again.'; busy = false; };
  ...
  function renderPhraseDrill(lesson, step, map) {
    ...
    <input class="${map?.classes?.input || "jp-input"}" placeholder="Type in JapaneseΓÇª" />
    <div class="${map?.classes?.romaji || "romaji"} text-gray-500 mb-1"></div>
    <div class="mt-2 flex items-center gap-2">
      <button class="btn btn-primary" data-act="check">Check</button>
      <button class="btn btn-amber"   data-act="speak">≡ƒöè Speak</button>
      <button class="btn btn-ghost"   data-act="alt">Γåö Variations</button>
      <button class="btn btn-dark"    data-act="next">Next Γ₧í</button>
    </div>
  ...
  `;
    ...
    function doCheck() {
      ...
      elHint.textContent = ok ? "" : `Hint: starts with πÇî${toHira(want).slice(0, 2)}πÇì`;
  ...
  function renderVariations(lesson, step, map) {
    ...
    <div class="flex items-center gap-2 mb-3">
      <button class="btn btn-primary" data-act="shuffle">≡ƒöÇ Shuffle</button>
      <button class="btn btn-ghost"   data-act="quiz">≡ƒº¬ Quiz me</button>
      <button class="btn btn-ghost"   data-act="showall">≡ƒôÜ Show all</button>
    </div>
  ...
  `;
    ...
    function renderList(arr) {
      ...
      arr.forEach(v => {
        ...
        line.innerHTML = `
        <div class="text-xs text-gray-500">${(v.tags || []).join(' ΓÇó ') || 'general'}</div>
        <div class="font-medium">${v.jp}</div>
        <div class="text-gray-500">${v.romaji || ''}</div>
        <div class="text-gray-600">${v.en || ''}</div>
        <div class="mt-1">
          <button class="btn btn-amber" data-jp="${v.jp}">≡ƒöè Listen</button>
        </div>
      `;
    ...
    function startQuiz() {
      ...
      <div class="grid gap-2">
        ${options.map((o, i) => `
          <button class="btn btn-ghost text-left" data-i="${i}">ΓÇó ${o.jp}
    ...
    `;
      ...
      [...quiz.querySelectorAll('[data-i]')].forEach(btn => {
        btn.addEventListener('click', () => {
          ...
          feedback(map, ok ? 'Nice choice!' : 'Not quiteΓÇölisten again and try another.', ok);
          mascotPulse && mascotPulse(ok ? 'mascot-celebrate' : 'mascot-confused', ok ? 1200 : 800);
  ...
  function renderBilingualScene(lesson, step, map) {
    ...
    <div class="flex items-center gap-2 mb-3">
      <button class="btn btn-primary" data-act="play">Γû╢ Play</button>
      <button class="btn btn-ghost"   data-act="pause">ΓÅ╕ Pause</button>
      <button class="btn btn-ghost"   data-act="prev">Γ¼à Prev</button>
      <button class="btn btn-dark"    data-act="next">Next Γ₧í</button>
  ...
  `;
    ...
    segs.forEach((g, i) => {
      ...
      line.innerHTML = `
      <div class="text-xs text-gray-500">${g.speaker} ΓÇó ${g.lang === 'ja' ? 'Japanese' : 'English'}</div>
    ...
    setStatus(map, 'Listen through the mini-scene. Use Γû╢ or Autoplay.');
    feedback(map, '', true);
  }
  ...
  function checkCloze(map) {
    ...
    blocks.forEach(block => {
      ...
      inputs.forEach(inp => {
        ...
        const got = inp.value || "";
        // Accept kana reading for Θíÿ if typed as πüèπü¡πüîπüäπüùπü╛πüÖ
        const acceptAlt = /Θíÿ/.test(expected) && H.toHira(got) === "πüèπü¡πüîπüäπüùπü╛πüÖ";
  ...
  function checkTranslate(lesson, step, map) {
    ...
    cards.forEach(card => {
      ...
      const readingKana = sentenceReadingHira(s); // e.g., πüèπü¡πüîπüäπüùπü╛πüÖ
      const correct =
        norm(s.jp || "") === norm(got) ||
    ...
    feedback(map, allOk ? "NiceΓÇöperfect translations!" : "Check the highlighted answers.", allOk);
    if (allOk) mascotPulse && mascotPulse("mascot-celebrate", 1200);
    else mascotPulse && mascotPulse("mascot-confused", 800);
    return allOk;
  }
  ...
  function run(lesson, map) {
    ...
    const initSpeedUI = () => {
      ...
      if (label) label.textContent = `${rate.toFixed(1)}├ù`;
    };
