export function initNinjaSlice(config) {
  ...
  // Remove full stops (Japanese/ASCII) for banners/reveals
  function stripFullStops(text){
    try { return (text || '').toString().replace(/[\u3002\uFF0E\uFF61\.]+/g, ''); } catch { return text || ''; }
  }
...
function groupForIndex(idx){
  ...
  function presentStageReveal(phraseText, romajiText, onDone) {
    try {
      let phrase = (phraseText || '').toString().trim();
      ...
      let romaText = (romajiText || '').toString();
      const tPhrase = cascade(phraseEl, phraseToShow, Math.round(stageCascadeStepMs * 1.1));
      ...
      try {
        const toSay = speakRomaji && romaText ? romaText : phraseToShow;
        speakJA(toSay).catch(()=>{});
      } catch {}
  ...
  function showStageBanner(number, phraseText, romajiText, englishText){
      try { if (revealOverlay) revealOverlay.style.display = 'none'; } catch {}
    try{
      ...
      cascade(phEl, stripFullStops(phraseText || ''), stageCascadeStepMs);
      cascade(roEl, stripFullStops(romajiText || ''), Math.round(stageCascadeStepMs * 0.8));
